    ...
    <div >
        ...
        <div class="flex flex-row">
                ...
                <div v-if="$parent.$props.config.toolbar && $parent.$props.config.toolbar.show != false" class="my-2 ml-2 bg-gray-100 w-38	p-2 rounded-lg flex flex-row justify-center items-center">
                   <div> <i @click="deleteElements()" class="confirm text-gray-500 far fa-trash-alt cursor-pointer hover:text-red-500 transition-colors	duration-150 "></i></div>
...
<!--                    <div><i  @click="exportElements()" class="ml-2     text-gray-500 fas fa-file-export cursor-pointer hover:text-green-500 transition-colors	duration-150 "></i></div>-->
                             ...
                             <!--   per page selection   -->
                <div v-if="$parent.$props.config.perPage.show != false" class="my-2 ml-2 bg-gray-100 w-40		p-2 rounded-lg flex flex-row justify-center items-center">
                    ...
                    <div class=" relative w-full">
                        <select @change="rowsChange()" v-model="perPage" class="focus:ring-blue-400 focus:border-blue-400 block appearance-none w-full bg-white border border-gray-400 hover:border-gray-500 px-4 py-2 pr-8 rounded shadow leading-tight focus:outline-none focus:shadow-outline">
                            <option v-for="page in $parent.config.perPage.counts" :value="page">{{ page }}</option>
        ...
        <!--   select filtration   -->
        <div v-if="this.$parent.$props.config.filter && this.$parent.$props.config.filter.selection && this.$parent.$props.config.filter.selection.show != false"  class="flex flex-row justify-start items-center">

            <div v-for="select in this.$parent.$props.config.filter.selection.data"  class="my-2 ml-2 bg-gray-100 w-48 p-2 rounded-lg flex flex-row justify-center items-center">
